data {
    ...
    // correlation
    int<lower=2> n_grappling;
    int<lower=2> n_striking;
    array[n_grappling] int I_grappling;
    array[n_striking] int I_striking;
}
...
parameters {
    ...
    // grappling
    vector[n_grappling] mu_grappling;
    vector<lower=0>[n_grappling] tau_grappling;
    cholesky_factor_corr[n_grappling] L_grappling;

    // striking
    vector[n_striking] mu_striking;
    vector<lower=0>[n_striking] tau_striking;
    cholesky_factor_corr[n_striking] L_striking;
}
...
transformed parameters {
    ...
    {
        matrix[n_grappling, n_grappling] L_grappling_cov = diag_pre_multiply(tau_grappling, L_grappling);
        matrix[n_striking, n_striking] L_striking_cov    = diag_pre_multiply(tau_striking, L_striking);

        for (n in 1:n_fighters - 1) {
            lambda[I_grappling, n + 1] = mu_grappling + L_grappling_cov * lambda_free[I_grappling, n];
            lambda[I_striking, n + 1]  = mu_striking + L_striking_cov * lambda_free[I_striking, n];
        }
    }
...
model {
    ...
    mu_grappling  ~ normal(0, sqrt(2) * 2.5);
    tau_grappling ~ normal(0, sqrt(2) * 2.5);
    L_grappling   ~ lkj_corr_cholesky(4.0);

    mu_striking   ~ normal(0, sqrt(2) * 2.5);
    tau_striking  ~ normal(0, sqrt(2) * 2.5);
    L_striking    ~ lkj_corr_cholesky(4.0);
    ...
    for (n in 1:N) {
        ...
        for (k in 1:K1) {
            if (T1[1, k, n] > 0) {
                y1[1, k, n] ~ poisson_log(eta1_blue[k, n]);
            }

            if (T1[2, k, n] > 0) {
                y1[2, k, n] ~ poisson_log(eta1_red[k, n]);
            }
        }
        ...
        for (k in 1:K2) {
            if (T2[1, k, n] > 0) {
                y2[1, k, n] ~ poisson_log(eta2_blue[k, n]);
            }

            if (T2[2, k, n] > 0) {
                y2[2, k, n] ~ poisson_log(eta2_red[k, n]);
            }
        }
        ...
        for (k in 1:K3) {
            if (N3[1, k, n] >= n3[1, k, n]) {
                n3[1, k, n] ~ binomial_logit(N3[1, k, n], alpha3_blue[k, n]);
            }

            if (N3[2, k, n] >= n3[2, k, n]) {
                n3[2, k, n] ~ binomial_logit(N3[2, k, n], alpha3_red[k, n]);
            }
        }
        ...
        for (k in 1:K4) {
            if (N4[1, k, n] >= n4[1, k, n]) {
                n4[1, k, n] ~ binomial_logit(N4[1, k, n], alpha4_blue[k, n]);
            }

            if (N4[2, k, n] >= n4[2, k, n]) {
                n4[2, k, n] ~ binomial_logit(N4[2, k, n], alpha4_red[k, n]);
            }
        }
        ...
        for (k in 1:K5) {
            if (N5[1, k, n] >= n5[1, k, n]) {
                n5[1, k, n] ~ binomial_logit(N5[1, k, n], alpha5_blue[k, n]);
            }

            if (N5[2, k, n] >= n5[2, k, n]) {
                n5[2, k, n] ~ binomial_logit(N5[2, k, n], alpha5_red[k, n]);
            }
        }
        ...
        for (k in 1:K6) {
            if (N6[1, k, n] >= n6[1, k, n]) {
                n6[1, k, n] ~ binomial_logit(N6[1, k, n], alpha6_blue[k, n]);
            }

            if (N6[2, k, n] >= n6[2, k, n]) {
                n6[2, k, n] ~ binomial_logit(N6[2, k, n], alpha6_red[k, n]);
            }
        }
    }
}
...
generated quantities {
    ...
    for (n in 1:N) {
        ...
        for (k in 1:K1) {
            if (T1[1, k, n] > 0) {
                y1_hat[1, k, n] = poisson_log_rng(eta1_blue[k, n]);
            } else {
                y1_hat[1, k, n] = -1;
            }

            if (T1[2, k, n] > 0) {
                y1_hat[2, k, n] = poisson_log_rng(eta1_red[k, n]);
            } else {
                y1_hat[2, k, n] = -1;
            }
        }
        ...
        for (k in 1:K2) {
            if (T2[1, k, n] > 0) {
                y2_hat[1, k, n] = poisson_log_rng(eta2_blue[k, n]);
            } else {
                y2_hat[1, k, n] = -1;
            }

            if (T2[2, k, n] > 0) {
                y2_hat[2, k, n] = poisson_log_rng(eta2_red[k, n]);
            } else {
                y2_hat[2, k, n] = -1;
            }
        }
        ...
        // strike accuracy
        for (k in 1:K3) {
            n3_hat[1, k, n] = binomial_rng(N3[1, k, n], inv_logit(alpha3_blue[k, n]));
            n3_hat[2, k, n] = binomial_rng(N3[2, k, n], inv_logit(alpha3_red[k, n]));
        }

        // knockdowns and knockouts
        for (k in 1:K4) {
            n4_hat[1, k, n] = binomial_rng(N4[1, k, n], inv_logit(alpha4_blue[k, n]));
            n4_hat[2, k, n] = binomial_rng(N4[2, k, n], inv_logit(alpha4_red[k, n]));
        }

        // grappling accuracy
        for (k in 1:K5) {
            n5_hat[1, k, n] = binomial_rng(N5[1, k, n], inv_logit(alpha5_blue[k, n]));
            n5_hat[2, k, n] = binomial_rng(N5[2, k, n], inv_logit(alpha5_red[k, n]));
        }

        // strike target
        for (k in 1:K6) {
            n6_hat[1, k, n] = binomial_rng(N6[1, k, n], inv_logit(alpha6_blue[k, n]));
            n6_hat[2, k, n] = binomial_rng(N6[2, k, n], inv_logit(alpha6_red[k, n]));
        }
    }
}